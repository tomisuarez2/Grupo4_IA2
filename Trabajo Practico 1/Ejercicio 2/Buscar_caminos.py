...
with open('distancias.txt', 'w') as archivo:
    ...
    for i in range(len(lugar_estantes)):
        for j in range(len(lugar_estantes)):
            if i != j: # Si los elementos son distintos:
                ...
                # Guardamos en el .txt.
                archivo.write(str(claves[i]) + ',' + str(claves[j]) + '=' + str(distancia) + '\n')

    # Calculamos la distancia de los estantes a la bahía. El indice 0 en la matriz corresponde a la bahía.
    ...
    for j in range(len(lugar_estantes)):
        ...
        distancia = buscar(comienzoX, comienzoY, finalX, finalY)
        # Guardamos en el .txt.
        archivo.write(str(0) + ',' + str(claves[j]) + '=' + str(distancia) + '\n')
